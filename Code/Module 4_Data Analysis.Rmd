...
We focus only on shark records, with catches summarised by Area and Year.
...
For better visual comprehenssion I have decided to use R , to make geographical representation of the data for the year 2020 using the maps package .
...
# Filter shark data for a single year (e.g. 2020)
shark2010 <- shark_tidy %>% filter(Year == 2010)

# Merge shark catches with coordinates
shark_map <- left_join(shark2010, area_coords, by = "Area")
...
ggplot() +
  ...
  scale_colour_viridis_c() +
  coord_fixed(xlim = c(140, 155), ylim = c(-30, -10)) +
  labs(title = "Shark catches by area (2010)",
       x = "Longitude", y = "Latitude",
       size = "Sharks", colour = "Sharks") +
  theme_minimal()
```
